...
class Im_Invisible_Can_You_See_Me {
	...
	public function validate() {
		...
		if ( empty( $_POST['g-recaptcha-response'] ) ) {
			$bp->signup->errors['signup_password_confirm'] = esc_html__( 'Please turn on javascript to register for an account', 'buddypress' );
		}
		...
		if ( ! isset( $verify->success ) ) {
			$bp->signup->errors['signup_password_confirm'] = esc_html__( 'Something went wrong when verifying the spam prevention response', 'buddypress' );
		}

		if ( false === $verify->success ) {
			$bp->signup->errors['signup_password_confirm'] = esc_html__( 'You did not pass our spam prevention check', 'buddypress' );
		}
	}
